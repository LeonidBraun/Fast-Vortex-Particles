...
void insert(const float& x, const float& y, const float& G, const float& h,
            Branch& branch, const float xm, const float ym, const float D) {
  if (D * D < 0.01f * h) {
    float dx = x - xm;
    float dy = y - ym;
    branch.x += dx * G;
  ...
  insert(x, y, G, h, branch.child[s], xm + sign(dx) * 0.25f * D,
         ym + sign(dy) * 0.25f * D, D * 0.5f);
  return;
}

void evaluate(float& u, float& v, const float& x, const float& y,
              const float& h, Branch branch, const float xm, const float ym,
              const float D) {
  if (branch.child == NULL) {
    float dx = x - branch.x;
    float dy = y - branch.y;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    evaluate(u, v, x, y, h, branch.child[i], xm + dir_x * D, ym + dir_y * D,
             0.5f * D);
  }
  return;
}
...
Tree createTree(const std::vector<float>& x, const std::vector<float>& G,
                const float& h) {
  ...
  Tree tree =
      Tree(0.5f * (xmax + xmin), 0.5f * (ymax + ymin),
           ((xmax - xmin) > (ymax - ymin)) ? (xmax - xmin) : (ymax - ymin));
  tree.root->x = x[0];
  tree.root->y = x[1];
  tree.root->G = G[0];
  for (size_t i = 1; i < G.size(); i++) {
    insert(x[2 * i + 0], x[2 * i + 1], G[i], h, *tree.root, tree.xm, tree.ym,
           tree.D);
  }
  return tree;
}
...
void calcVelandStep(std::vector<float>& u, std::vector<float>& x,
                    const std::vector<float>& G, const float& h, Tree& tree,
                    const float& dt) {
